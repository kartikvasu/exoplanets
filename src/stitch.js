...
node.append("circle")
  ...
  d3.json("../interaction-data.json", function(json) {
    
      var circle = vis.append('circle')
      .attr('r', function(d) { return 10; })
      .attr('id', 'cursor')
      .attr('cx', function() { return 276; })
      .attr('cy', function() { return 10.125; })
      .style('fill', 'red');
    
      var i = 0;

      circle.transition()
      .attr('cx', json.sessions[i]['x'])
      .attr('cy', json.sessions[i]['y'])
      .each('end', repeat);
    
      function repeat () {
        if(json.sessions[i]['type'] == "mouseover") {
          console.log($("svg").find("[x='" + json.sessions[i]['x'] + "'][y='"+ json.sessions[i]['y']  + "']"));
          $("svg")
          .find("[x='" + json.sessions[i]['x'] + "'][y='"+ json.sessions[i]['y']  + "']")
          .triggerSVGEvent('mouseover');
        }
        if(json.sessions[i]['type'] == "mouseout") {
          console.log('mouseout' + $("svg").find("[x='" + json.sessions[i]['x'] + "'][y='"+ json.sessions[i]['y']  + "']"));
          $("svg")
          .find("[x='" + json.sessions[i]['x'] + "'][y='"+ json.sessions[i]['y']  + "']")
          .triggerSVGEvent('mouseout');          
        }
        // this is used to trigger an SVG Event
        // for the purpose of invoking a hover, or a mousemove
